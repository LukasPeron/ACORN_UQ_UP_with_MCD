...
def load_reconstruction_df(graph):
    ...
    pids[graph.track_edges[0]] = graph.track_particle_id if hasattr(graph, "track_particle_id") else graph.particle_id
    pids[graph.track_edges[1]] = graph.track_particle_id if hasattr(graph, "track_particle_id") else graph.particle_id

    return pd.DataFrame(
        {"hit_id": hit_id, "track_id": graph.edge_track_labels if hasattr(graph, "edge_track_labels") else graph.labels, "particle_id": pids}
    )
...
def load_particles_df(graph, sel_conf: dict):
    ...
    cols = {
        "particle_id": graph.track_particle_id if hasattr(graph, "track_particle_id") else graph.particle_id,
        "pt": graph.track_particle_pt if hasattr(graph, "track_particle_pt") else graph.pt,
        "eta": graph.hit_eta[graph.track_edges[0]]
    }
    # if "track_particle_eta" in graph:
    #     cols["eta"] = graph.track_particle_eta if hasattr(graph, "track_particle_eta") else graph.eta
...
def plot_eff(
    particles, config, var: str, varconf: Dict, save_path="track_reconstruction_eff_vs_XXX"
):
    ...
    np.savetxt(
        ...
        eff)
    np.savetxt(
        save_path + "_xvals.txt",
        xvals)
    np.savetxt(
        save_path + "_xerrs.txt",
        xerrs)
    np.savetxt(
        save_path + "_err.txt",
        err)

    print(f"Save efficiency to {save_path}_XXX.txt")

    # Add x and y labels
    ax.set_xlabel(varconf.get("x_label", "x_label is None"), fontsize=14, ha="right", x=0.95)
    ...
    atlasify("1400 events", 
        f"{'Calibrated' if config.get('calibration', False) else 'Uncalibrated'}" + "\n"
        r"$p_T > 1$GeV, $|\eta| < 4$",
        enlarge=1,
    )
    ...
    fig.savefig(save_path_svg := save_path.replace(".png", ".svg"))
