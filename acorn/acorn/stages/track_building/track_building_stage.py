...
class TrackBuildingStage:
    ...
    def tracking_efficiency(self, plot_config, config):
        ...
        res_fname = os.path.join(
            self.hparams["stage_dir"], f"{'cal_res' if config.get('calibration', False) else 'uncal_res'}/",
            f"results_summary_{self.hparams.get('matching_style', 'ATLAS')}_{'Calibrated' if config.get('calibration', False) else 'Uncalibrated'}.txt",
        )
        ...
        for var, varconf in plot_config["variables"].items():
            save_path = os.path.join(
                ...
                f"{'cal_res' if config.get('calibration', False) else 'uncal_res'}/track_reconstruction_eff_vs_{var}_{self.hparams.get('matching_style', 'ATLAS')}_{'Calibrated' if config.get('calibration', False) else 'Uncalibrated'}",
            )
...
class GraphDataset(Dataset):
    ...
    def get(self, idx):
        ...
        event = torch.load(event_path, map_location=torch.device("cpu"), weights_only=False)
