...
def evaluate(config_file, verbose=None, checkpoint=None, dataset="valset"):
    ...
    if issubclass(stage_module, LightningModule):
        ...
        checkpoint_config["input_dropout"] = config.get("input_dropout", 0.0)
        checkpoint_config["hidden_dropout"] = config.get("hidden_dropout", 0.0)
    ...
    if issubclass(stage_module, LightningModule) and config["n_train"]>0:
        stage_module.evaluate(config, checkpoint_path) #only for filter & GNN evaluation 
    else:
        print("Use no checkpoint for evaluation !")
